...
const deleteTransaction = async (transaction) => {
  ...
  try {
    const res = await fetch(
      `${API_URL}/trips/${tripId}/transactions/${transaction.id}`,
      {
        method: "DELETE",
      }
    );
...
<template>
  <div v-if="trip" class="max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-6">
      <div>
        <div class="flex items-center text-sm text-gray-500 mb-1">
          ...
          <router-link to="/setup" class="hover:text-indigo-600"
            >旅程管理</router-link
          >
        </div>
      ...
      <div class="space-x-2">
        <button
          @click="router.push({ name: 'transaction', params: { tripId } })"
          class="bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded"
        >
          + 新增消費
        </button>
        <button
          @click="router.push({ name: 'settlement', params: { tripId } })"
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
      ...
      >
        <h3 class="text-gray-500 text-sm uppercase font-bold">
          MVP (墊付最多)
        </h3>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden mb-8">
      ...
      <div class="p-6">
        <div
          v-for="(amount, name) in memberSpends"
          :key="name"
          class="flex justify-between items-center border-b border-gray-100 py-2 last:border-0"
        >
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <div class="p-6">
        <ul v-if="trip.transactions.length > 0">
          <li
            v-for="(t, i) in trip.transactions.slice().reverse()"
            :key="t.id || i"
            class="mb-3 pb-3 border-b border-gray-100 last:border-0 last:pb-0 last:mb-0 group"
          >
            <div class="flex justify-between items-start">
              <div>
                ...
                <div class="text-sm text-gray-500">
                  由
                  <span class="font-medium text-gray-700">{{ t.payer }}</span>
                  支付
